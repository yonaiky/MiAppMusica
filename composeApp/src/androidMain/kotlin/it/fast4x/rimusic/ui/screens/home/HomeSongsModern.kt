                                ...
                                .combinedClickable(
                                    onClick = {
                                        ...
                                        if (items.filter { it.song.likedAt != -1L }.isNotEmpty()) {
                                            val itemsLimited =
                                                if (items.filter { it.song.likedAt != -1L }.size > maxSongsInQueue.toInt()) items.filter { it.song.likedAt != -1L }
                                                    .shuffled()
                                                    .take(maxSongsInQueue.toInt()) else items.filter { it.song.likedAt != -1L }
                                            binder?.stopRadio()
