...
fun PlayerActionBar(search: SettingEntrySearch ) {
    ...
    if (!showLyricsThumbnail) {
        ...
        if ( Settings.PLAYER_SHOW_TOTAL_QUEUE_TIME.value && search.contains( R.string.show_total_time_of_queue ) )
            SettingComponents.BooleanEntry(
                Settings.PLAYER_IS_QUEUE_DURATION_EXPANDED,
                R.string.show_total_time_of_queue
            )

        if ( search.contains( R.string.titleartist ) )
            SettingComponents.BooleanEntry(
        ...
        if ( search.contains( R.string.controls ) )
            SettingComponents.BooleanEntry(
                Settings.PLAYER_IS_CONTROLS_EXPANDED,
                R.string.controls
            )

        if( Settings.PLAYER_STATS_FOR_NERDS.value
            && (!(showThumbnail && Settings.PLAYER_TYPE.value == PlayerType.Essential))
            && search.contains( R.string.statsfornerds )
        )
            SettingComponents.BooleanEntry(
        ...
        if (
            Settings.PLAYER_ACTION_ADD_TO_PLAYLIST.value
            || Settings.PLAYER_ACTION_OPEN_QUEUE_ARROW.value
            || Settings.PLAYER_ACTION_DOWNLOAD.value
            || Settings.PLAYER_ACTION_LOOP.value
            || Settings.PLAYER_ACTION_SHOW_LYRICS.value
            || Settings.PLAYER_ACTION_TOGGLE_EXPAND.value
            || Settings.PLAYER_ACTION_SHUFFLE.value
            || Settings.PLAYER_ACTION_SLEEP_TIMER.value
            || Settings.PLAYER_ACTION_SHOW_MENU.value
            || Settings.PLAYER_ACTION_OPEN_EQUALIZER.value
            || Settings.PLAYER_ACTION_DISCOVER.value
            || Settings.PLAYER_ACTION_TOGGLE_VIDEO.value
        ...
        if( Settings.PLAYER_SHOW_NEXT_IN_QUEUE.value
            && Settings.PLAYER_IS_ACTIONS_BAR_EXPANDED.value
            && search.contains( R.string.miniqueue )
        )
            SettingComponents.BooleanEntry(
    ...
    if( search.contains( R.string.title_playback_speed ) )
        SettingComponents.BooleanEntry(
